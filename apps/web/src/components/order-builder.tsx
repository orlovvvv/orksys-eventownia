...
const orderEventSchema = z.object({
  ...
  street: z.string().trim().min(3, "Podaj adres wydarzenia."),
...
export function OrderBuilder() {
  ...
  const submitOrder = useMutation(
    trpc.orders.submit.mutationOptions({
      onSuccess: (data) => {
        clearCart();
        void navigate({ to: "/rezerwacja/$publicToken", params: { publicToken: data.publicToken } });
      },
      onError: (error) => {
  ...
  useEffect(() => {
    ...
    void navigate({ to: "/wynajem", search: search.date ? { date: search.date } : {}, replace: true });
      ...
      <CardContent className="flex flex-col gap-4">
        {rows.length === 0 ? (
          <div className="flex flex-col items-center gap-4 py-10 text-center">
            ...
            <Button render={<Link to="/produkty" />}>Przejdź do katalogu</Button>
          </div>
        ) : null}
        {rows.map((row) =>
    ...
    <div className="grid gap-4 rounded-xl bg-muted p-4 sm:grid-cols-[6rem_1fr_auto] sm:items-center">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {row.product.pricing?.quoteMode === "automatic" ? `${formatMoney(row.product.pricing.basePriceGrosz)} bazowo` : "Wycena indywidualna"}
        </p>
      </div>
